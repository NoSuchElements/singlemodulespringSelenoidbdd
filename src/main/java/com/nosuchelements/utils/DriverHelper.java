...
import com.nosuchelements.constants.ColorConstants;
import com.nosuchelements.constants.DataDictionary;
...
//    private final DriverWait driverWait;
    ...
    @Autowired
    private final WaitHelper driverWait;
    

    @Autowired
    public DriverHelper(DriverManager driverManager, WaitHelper driverWait) {
    ...
    public void click(WebElement element) throws NoSuchFieldException {
        try {
            ...
            log.warn("Could not click on the element");
    ...
    public void rightClick(By locator) throws NoSuchFieldException {
        ...
        try {
            ...
            log.warn("Could not click on the element : " + element);
            throw new RetryException("Could not click on the element : " + element);
        }
    }
...
//                    _by.toString()));
    ...
    @Retryable(maxAttempts = 3, backoff = @Backoff(delay = 500), retryFor = { RetryException.class })
    public List<WebElement> getAllVisibleElements(String xpath) throws Exception {
...
//                _by.toString()));
            	...
            	do {
            		eCount = 0;
            		List<WebElement> els = driverManager.getDriver().findElements(By.xpath(xpath));
            		if (els != null) {
            			eCount = els.size();
            	...
            	if (elems.size() == 0) {
            		if (eCount == 0) {
            			throw new NoSuchElementException("xpath=" + xpath);
            		}
            		throw new Exception("xpath=" + xpath);
            	}
...
//    	log.info(String.format("Attempting to find and return the WebElement(s) with the following By value: [%s]", _by.toString()));
     ...
     * @throws Exception 
     */
    public WebElement getAnyElementWhenVisible(List<WebElement> elementsList) throws Exception {
//    	log.info(String.format("Attempting to find and return the WebElement with the following By value: [%s]",  _by.toString()));
    	...
    	if (el == null) {
    		if (eCount == 0) {
    			throw new NoSuchElementException("Count is zero");
    		}
...
//    	log.info(String.format("Navigating to the following URL: [%s]", _url));
        driverManager.getDriver().navigate().to(_url);
    }
...
//    	log.info(String.format("Switching to tab using title value: [%s]..", _tab));
    	driverManager.getDriver().switchTo().window(_tab);
    }
...
//    	log.info(String.format("Switching to tab using index value: [%d]..", _tab));
    	driverManager.getDriver().switchTo().window(Integer.toString(_tab));
    }
...
//			log.error("Caught exception screenShotName=" + fname);
        	List<WebElement> els = driverManager.getDriver().findElements(By.xpath("//button[@class='con-x']"));
...
//        	log.info("Caught exception trying to click element, will retry in 1 second incase it is transient");
        	...
        	((JavascriptExecutor) driverManager.getDriver()).executeScript("return window.scrollBy(0,100);");
        	Thread.sleep(500);
...
//    	log.info(String.format("Searching for an entry in the data dictionary for input: %s", _input));
    	if (_input.startsWith("dd:normalized:")) {
        	_input=_input.replace("normalized:", "");
            String[] parts   = _input.split("dd:");
            return DataDictionary.getMessageFromDataDictionary(lang.toString().substring(0,1), parts[1]);
		}
    	else if (_input.startsWith("dd:")) {
//        	log.info("Object is prepended with \"dd:\" - Returning associated value..");
            String[] parts    = _input.split("dd:");
            //todo inline this here
            return DataDictionary.getMessageFromDataDictionary(lang.toString().substring(0,1), parts[1]);
...
//    	log.info("Entering validateUpdatedSuccessAlert");
    	WebElement locUpdateSuccAlert = driverManager.getDriver().findElement(By.xpath(locUpdateSuccessAlert));
...
//    	WebElement el = getElementWhenVisible(By.xpath(locUpdateSuccessAlertButton));   	
    	...
    	if (driverManager.isMobile()) {
    		WebElement el = driverManager.getDriver().findElement(By.xpath("//button[@id='success-error-banner']"));
    		clickElementJS(el);
    	} else {
    		WebElement el =driverManager.getDriver().findElement(By.xpath("//button[@id='success-error-banner']"));
    		clickElement(el);
...
//    	log.info("Entering validateUpdatedFailureAlert");
    	...
    	WebElement el = driverManager.getDriver().findElement(By.xpath(locUpdateSuccessAlert));
    	validateBackgroundColor(ColorConstants.RED, el);
    	validateText(locUpdateSuccessAlertText, searchDataDictionary(msgKey)); 
    	WebElement alert = driverManager.getDriver().findElement(By.xpath("//*[@id=\"success-error-banner\"]/span[1]"));
    	clickElement(alert);    	
...
//        WebElement element   = getElementWhenVisible(_by);
    ...
    public void dismissHelper() {		
    	//Dismiss helper
		List<WebElement> helper = driverManager.getDriver().findElements(By.xpath("//*[@id=\"con-notification-title\"]"));		
...
//			log.info("Found popover");
			WebElement button = driverManager.getDriver().findElement(By.xpath("//*[@id=\"con-notification\"]/button"));
			try {
				button.click();
			} catch (Exception ex) {
//				log.info("Unable to click con-notification button");
  ...
  //Used to wait for page loading spinners for pages that load text early Eg. Settings/Payments titles
    public void waitForSpinnersInvisible() {
    	...
    	List<WebElement> spinners = driverManager.getDriver().findElements(By.xpath("//div[@class='spinner']"));  
...
//			log.info("Attempting to validate the tooltip line color");
		...
		public boolean verifyErrorMessage(WebElement locName, String errorMessage, String colour) throws Exception {
			...
			try {
				...
				Assert.assertEquals(actualmessage,expectedmessage,"Failed to validate error message");
...
//	    	log.info(String.format("Attempting to find and return the WebElement with the following By value: [%s]",  _by.toString()));
		...
		@Retryable(maxAttempts = 3, backoff = @Backoff(delay = 500), retryFor = { RetryException.class })
		 ...
		 public List<WebElement> getAllVisibleElements(By _by, int minVisibleCount) throws Exception {
//		    	log.info(String.format("Attempting to find and return all visible WebElement(s) with the following By value: [%s]",
//		                _by.toString()));
		                long cutoff = new Date().getTime() + 10000;
		            	List<WebElement> elems = new ArrayList<WebElement>();
		            	...
		            	do {
		            		eCount = 0;
		            		List<WebElement> els = driverManager.getDriver().findElements(_by);
		            		if (els != null) {
		            			eCount = els.size();
		            			for (WebElement e : els) {
		            				if (e.isDisplayed()) {
		            	...
		            	if (elems.size() == 0) {
		            		if (eCount == 0) {
		            			throw new NoSuchElementException("_by=" + _by.toString());
		            		}
		            		throw new Exception("_by=" + _by.toString());
		            	}
...
//		    	log.info(String.format("Attempting to find and return the WebElement with the following By value: [%s]",_by.toString()));
		    	...
		    	if (el == null) {
		    		...
		    		throw new ElementNotInteractableException("_by=" + _by.toString());
		    	}
...
//		        	log.info("Caught an ElementClickInterceptedException. Will try close info dialog and reclick", ex);
//		        	String fname = ScreenShot.takeScreenShot(driverManager.getDriver(), this.getClass().getName());
//					log.error("Caught exception screenShotName=" + fname);
		        	List<WebElement> els = driverManager.getDriver().findElements(By.xpath("//button[@class='con-x']"));
		        	if (els.size() > 0 && els.get(0).isDisplayed()) {
//		        		log.info("Clicking on possible dialog");
...
//		    	log.info(String.format("Clicking the given WebElement: [%s]",_visibleBy.toString()));
		        ...
		        try {
		        	scrollIntoView(element);
		        	driverManager.getDriver().findElement(_clickBy).click();
		        } catch(Exception ex) {
//		        	log.info("Caught exception trying to click element, will retry in 1 second incase it is transient");
		        	Thread.sleep(500);
		        	// Scroll down a bit in case another object like "Chat" is blocking the element
		    ...
		    public WebElement getActiveWebElement(By _by) throws ValidationException {
		    	dismissHelper();
		    	List<WebElement> allElements = driverManager.getDriver().findElements(_by);
...
//					log.info("Attempting to validate the tooltip line color");
				...
				public void switchParentWindow() throws Exception {
					try {
						...
						String firstWinHandle = driverManager.getDriver().getWindowHandle();
			 
						String winHandle = handles.iterator().next();
			 
						driverManager.getDriver().switchTo().window(firstWinHandle);
					} catch (Exception ex) {
						ex.printStackTrace();
				...
				public void switchChildWindow() throws Exception {
					try {
						...
						if (winHandle != firstWinHandle) {
							...
							driverManager.getDriver().switchTo().window(secondWinHandle);
						}
					} catch (Exception ex) {
						ex.printStackTrace();
						throw new Exception(ex);
